...
public enum WeaponState_fire { SearchTarget = 0, AttackToTarget } // 열거형 
public class FireTowerWeapon : MonoBehaviour
{
    [SerializeField]
    private FireTowerTemplate towerTemplate;
    [SerializeField]
...
    private Transform attackTarget = null;
    ...
    public float Damage => towerTemplate.wapons[level].damage_fire;
    public float Rate => towerTemplate.wapons[level].rate_fire;
    public float Range => towerTemplate.wapons[level].range_fire;
...
        float degree = Mathf.Atan2(dy, dx) * Mathf.Rad2Deg;
    ...
    private IEnumerator SearchTarget()
    {
        while (true)
        {
            ...
            for (int i = 0; i < enemySpawner.EnemyList.Count; ++i)
            {
                ...
                if (distance <= towerTemplate.wapons[level].range_fire && distance <= closetDistSqr)
                {
    ...
    private IEnumerator AttackToTarget()
    {
        while (true)
        {
            ...
            if (distance > towerTemplate.wapons[level].range_fire)
            {
                attackTarget = null;
                ChangeState(WeaponState.SearchTarget);
                break;
            }
            yield return new WaitForSeconds(towerTemplate.wapons[level].rate_fire);
            //공격
            spawnProjectile();
        }
    }
    ...
    public bool Upgrade() // 타워 템플릿 추가시 추가 
    {
        ...
        if (playerGold.CurrentGold < towerTemplate.wapons[level + 1].cost_fire)
        {
            return false;
        }
        // 타워 레벨 증가
        ...
        //골드 차감
        playerGold.CurrentGold -= towerTemplate.wapons[level].cost_fire;
